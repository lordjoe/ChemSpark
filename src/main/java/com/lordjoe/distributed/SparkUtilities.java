...
public class SparkUtilities implements Serializable {
    ...
    public static int getDefaultNumberPartitions() {
        ...
        return sparkConf.getInt("spark.default.parallelism", defaultNumberPartitions);
    }
...
//        logger.setLevel();
     ...
     * @return
     */
    public static void readSparkPropertiesResource(String fileName) {
        try {
            InputStream is =  SparkUtilities.class.getResourceAsStream(fileName);
           sparkProperties.load(is);  // read spark properties
        }
        catch (IOException e) {
            throw new RuntimeException(" bad spark properties file " + fileName);
